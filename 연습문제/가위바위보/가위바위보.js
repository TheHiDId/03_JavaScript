...
function judgeWinDrawLose(userChoice, comChoice) {
  switch(userChoice) {
    case 0: 
      if(comChoice == 2) {
        win();
...
function randomSRP() {
  const comChoice = Number(Math.floor(Math.random() * 3));
...
gameStartBtn.addEventListener("click", () => {
  ...
  const playRound = (userChoice) => {
    ...
    const comChoice = randomSRP();
    const result = judgeWinDrawLose(userChoice, comChoice);

    updateRecord(result, userChoice, comChoice);
  };
